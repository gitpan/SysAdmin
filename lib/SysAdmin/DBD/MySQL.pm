...
sub fetchTable {
	...
	if($db_select){
		if($db_select =~ /select\s.*/i){
			...
			my $dbh = DBI->connect("dbi:mysql:dbname=$db_database;host=$db_host;port=$db_port;","$db_username","$db_password") or die "Could not connect to $db_database";;
...
code apstraction to a minimum. This class acts as a sub class for DBD
objects.
